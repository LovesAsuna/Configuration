...
if exist Bungeecord.jar set jar=BungeeCord.jar && set server=BungeeCord && set status=bc && for /f "tokens=3 delims=:" %%i in ('findstr /rc:"\<host\>" config.yml') do (
  set port=%%i
) &&  goto Bungeecord
rem 读取服务端核心
...
goto reloadnode
:setbcmemory
echo 检测到为Bungeecord模式 & title %server%控制端 & echo 请设置最大内存和最小内存!
set /p max=最大内存(Max):
set /p min=最小内存(Min):
md %file%modules & cls
